...
cars_table_module <- function(input, output, session) {
  ...
  output$car_table <- renderDT({
    ...
    datatable(
      out,
      rownames = FALSE,
      colnames = c('EAL Number', '# of Users', 'CI/CD Enabled', 'AI Enabled',
                   'NPS', 'Weekly Hours for Maintenance', 'Integration Count', 'Weekly Failure Rate',
                   'Progamming Language Used', 'Home Grown/Third Party', 'Months Since Deploy Date', 
                   'Weeks Until Contract Expired', 'Created At',
      ...
      options = list(
        ...
        buttons = list(
          list(
            extend = "excel",
            text = "Download",
            title = paste0("EAL_Database-", Sys.Date()),
            exportOptions = list(
